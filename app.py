...
# Para testar sem derrubar o worker, deixe poucas lojas.
# Quando estabilizar, troque USE_TEST_DEALERS para False.
USE_TEST_DEALERS = True
TEST_DEALER_NAMES = {
    "Jeep Dahruj",
    "Divena Jeep",
    "Amazonas Jeep",
...
ACTIVE_DEALERS = [d for d in DEALERS if d["name"] in TEST_DEALER_NAMES] if USE_TEST_DEALERS else DEALERS
...
def fetch_detail_page(detail_url: str) -> dict[str, str]:
    try:
        ...
        body_text = normalize_spaces(soup.get_text(" ", strip=True)[:12000])

        price_match = re.search(r"R\$\s?[\d\.,]+", body_text, re.I)
        ...
        return {
            ...
            "version": infer_version(body_text),
            "year": "/".join(str(y) for y in extract_years(body_text)[:2]) or "N/I",
        }
    except Exception:
        return {}
...
def fetch_html_fallback(dealer: dict[str, Any]) -> list[Listing]:
    ...
    soup = BeautifulSoup(response.text[:400000], "html.parser")
    ...
    for anchor in anchors:
        ...
        try:
            ...
            should_fetch_detail = any(x in combined for x in ["renegade", "longitude", "t270", "turbo"])
            detail = {}

            if full_url != dealer["url"] and should_fetch_detail and detail_fetches < max_detail_fetches:
                detail = fetch_detail_page(full_url)
                detail_fetches += 1

            merged = normalize_spaces(" ".join([
                context,
                detail.get("title", ""),
            ...
            km = detail.get("km") or extract_km(merged)
            version = detail.get("version") or infer_version(merged)
            ...
            results.append(Listing(
                ...
                model="Jeep Renegade",
                version=version,
                year=detail.get("year") or year_label,
                price=price,
                price_label=format_price(price),
                km=km,
                color="N/I",
...
def run_scan(filters: dict[str, Any]) -> dict[str, Any]:
    ...
    for dealer in ACTIVE_DEALERS:
        ...
        try:
            if dealer["platform"] == "autoforce":
                items = fetch_autoforce(dealer)
                if not items:
                ...
                items = fetch_html_fallback(dealer)
    ...
    return {
        ...
        "messages": messages + [
            ...
            f"Modo de carga atual: {'teste reduzido' if USE_TEST_DEALERS else 'todas as lojas'}.",
        ],
    }
